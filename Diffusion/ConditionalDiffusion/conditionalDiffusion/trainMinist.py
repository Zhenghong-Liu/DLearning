...
def train_mnist():
    ...
    ddpm = DDPM(eps_model=ContextUnet(input_channels=1, output_channels=1, n_classes=n_classes), betas=(1e-4, 0.02), n_T=n_T,
                drop_prob=0.1)
    ddpm.to(device)
    ...
    for ep in range(n_epoch):
        ...
        with torch.no_grad():
            ...
            for w_i, w in enumerate(ws_test):
                x_gen, x_gen_store = ddpm.sample(n_sample, (1, 32, 32), device, guide_w=w, n_classes=n_classes)
